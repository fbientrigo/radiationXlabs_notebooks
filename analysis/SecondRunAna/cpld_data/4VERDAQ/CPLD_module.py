...
class CPLD:
    ...
    BAUDRATE = 115200
    TIMEOUT = 15 #P. buscar u.de medida + q implica


    ## Parámetros para guardar el output del archivo
    fname = "/cpld_data/cpld_data"  # file name, primero va la subcarpeta y
    ...
    MAXWAITCNT = 15   # esperamos leer hasta 15 veces 4bytes (ver método wait_initial_seq)
    MAXREADTIME = 100
    MAXWAITTIME = 30
    ...
    def wait_initial_seq(self):
        ...
        try:
            while res.decode() != "#": #secuencia inicial (válida) esperada
                ...
                sleep(2) # para qué?
                res = self.sp.read(1) # leemos otra secuencia de 1 bytes 
                ################################################################
...
if __name__ == '__main__':
    ...
    rootdir = "modified_code/data_test_CPLD" #28/07 testeo cpld
    ...
    CPLD("COM3", rootdir, log = log_cpld)
